...
def generate_random_al1_segment():
    allergy_type = random.choice(["DA", "FA"])
    allergy_name = fake.word()
    allergy_code = f"{random.randint(1, 999)}^{fake.word()}^{random.choice(['I10', 'I11'])}"
    reaction = fake.sentence()
    severity = random.choice(["M", "S", "U"])
...
def create_adt_message(patient_data):
    ...
    # Generate and add a random AL1 segment to the message
    al1_data = generate_random_al1_segment()
    al1 = msg.add_segment("AL1")
    al1.al1_1 = al1_data[1]
    al1.al1_2 = al1_data[2]
    al1.al1_3 = al1_data[3]
    al1.al1_4 = al1_data[4]
    al1.al1_5 = al1_data[5]
    al1.al1_6 = al1_data[6]
